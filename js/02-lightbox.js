...
const galeryMarkupString = galleryItems
  .map((item) => `<a class="gallery__item" href="${item.original}"><img class="gallery__image lazyload" src="${item.preview}" alt="${item.description}"/></a>`)
  .join('');
...
// Невдале намагання прибрати помилку в консолі при збільшенні/зменшенні зображення (скролі/зумі)
// lightbox.on('changed.simplelightbox', (e) => {
//   e.preventDefault();
// });
